...
#define SALTARRAYLIST "0123456789\
...
bool do_hash(char* input, uint8_t* out, bool salt) {

	int out_length = 256 / 8;		//byte size
	int in_length = 0;				//byte size
	int hash_bit = 256;				//bit(224,256,384,512)
	bool SHAKE = false;				//0 or 1
	...
	if (salt == true) {
		...
		for (i = 0; i < 5; i++) {
			salt[i] = (char)(SALTARRAYLIST[rand() % strlen(SALTARRAYLIST)]);
			printf("%c", salt[i]);
		}
	...
	printf("\n");
	result = sha3_hash(out, out_length, (uint8_t*)input, strlen(input), hash_bit, SHAKE);
...
bool find_first_20bit(char* input, uint8_t* out) {
	...
	result = do_hash(input, out, false);
	if (!result)
		return false;
